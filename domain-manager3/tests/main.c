...
extern uintptr_t _aes_start[];
extern uintptr_t _aes_end[];


extern uintptr_t _aaa_start[];
extern uintptr_t _aaa_end[];
...
void create_aes_domain(){
    P_LINE();
    int domain_flags = PK_INHERIT_KEY | PK_COPY_KEY;
    printf("aes_domain = dm_domain_create(domain_flags, 0)\n");
    aes_domain = dm_domain_create(domain_flags, 0);
    bool ret = dm_pkey_mprotect(aes_domain, _aes_start, SYM_SIZE(aes), PROT_EXEC | PROT_READ | PROT_WRITE, GET_DEFAULT_VKEY);
    assert(ret == 0);
}

void create_aaa_domain(){
    P_LINE();
    int domain_flags = PK_INHERIT_KEY | PK_COPY_KEY;
    printf("aaa_domain = dm_domain_create(domain_flags, 0)\n");
    aaa_domain = dm_domain_create(domain_flags, 0);
    bool ret = dm_pkey_mprotect(aaa_domain, _aaa_start, SYM_SIZE(aaa), PROT_EXEC | PROT_READ | PROT_WRITE, GET_DEFAULT_VKEY);
    assert(ret == 0);
}
