...
void test1_api() {
  ...
  // invalid flags
  printf("dm_domain_create(-1, 0) fail\n");
  ret = dm_domain_create(-1, 0);
  assert(ret == -1 && errno == EINVAL);
  printf("dm_domain_create(PK_OWNER_KEY, 0) fail\n");
  ret = dm_domain_create(PK_OWNER_KEY, 0);
  assert(ret == -1 && errno == EINVAL);
  printf("dm_domain_create(PK_COPY_KEY, 0) fail\n");
  ret = dm_domain_create(PK_COPY_KEY, 0);
  assert(ret == -1 && errno == EINVAL);

  printf("did1 = dm_domain_create(0, 0) \n");
  did1 = dm_domain_create(0, 0);
  assert(did1 > 0);

  printf("did2 = dm_domain_create(0, 0) \n");
  did2 = dm_domain_create(0, 0);
  assert(did2 > 0 && did1 != did2);

  printf("did3 = dm_domain_create(0, 0) \n");
