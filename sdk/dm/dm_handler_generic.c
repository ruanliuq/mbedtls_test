...
int D_CODE _dm_domain_create_underlocked(unsigned int flags){
	...
	_dm_domain * NewDoain = &(dm_data.domains[did]);
	NewDoain->used = true;
    
	// remember who created the domain
	if ( dm_trusted_tls.init ) {
		NewDoain->parent_did = CURRENT_DID;
	} else {
		NewDoain->parent_did = INVALID_DID;
	}
	...
	if(pkey < 0){
		...
		NewDoain->used = false;
	    memset(NewDoain, 0, sizeof(*NewDoain));
	    return -1;
	}
...
int D_CODE _init_root_domain(){
    ...
    // create root domain
	did_root = _dm_domain_create_underlocked(0);
	if (did_root == -1){
    ...
    did_for_exception_handler = _dm_domain_create_underlocked(0);
	if (did_root == -1){
		return cleanup_and_exit(0, NULL, unlock);
	}
...
//------------------------------------------------------------------------------
//   下面的 _dm_exception_handler_underlocked 函数需要用到的一些函数
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
int D_CODE _dm_domain_create(unsigned int flags){
    DEBUG_MPK("_dm_domain_create(0x%x)", flags);
    assert(dm_data.initialized);

    _dm_acquire_lock();
    int ret = _dm_domain_create_underlocked(flags);
    _dm_release_lock();
...
int D_CODE _dm_domain_free(int did){
    ...
    // TODO: implement me
    WARNING("_dm_domain_free Implement me");
...
//------------------------------------------------------------------------------
int D_CODE _test_simple_api(int a, int b, int c){
    int ret = a+b+c;
    DEBUG_MPK("_test_simple_api(a=%d, b=%d, c=%d). returning %d", a,b,c, ret);
    return ret;
}
