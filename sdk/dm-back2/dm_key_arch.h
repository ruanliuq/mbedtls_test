...
#define CSR_MISA 0x301         // 机器ISA信息
#define CSR_UMISA 0x047     // undefine
//------------------------------------------------------------------------------
// defines for PKRU  register
//------------------------------------------------------------------------------
// TEST IN DONKY
// #define CSR_UPKRU 0x046
#define CSR_UPKRU 0x048   // 定义的PKRU寄存器
//  TODO  异常代码为14
#define CAUSE_MPKEY_MISMATCH_FAULT 0xe
#define CSR_IRDI 0xBC0
...
#define CURRENT_ISA ({ assert(dm_trusted_tls.init); _read_pkru_reg().isa_id;})

static inline void isa_call(uint64_t value, int from) {
    uint64_t prev_did;
    asm volatile ("isacall %0, %1, %2" : "=r"(prev_did) : "r"(value), "i"(0));
    DEBUG_PRINTF("ISA call from %d to %d", from, (int)value);
    assert(prev_did == value);
}

//------------------------------------------------------------------------------
// TODO   static inline ==>  FORCE_INLINE
static inline void dm_print_reg_arch(pkru_config_t reg){
    fprintf(stderr,"pkru = 0x%zx, ", PKRU_TO_INT(reg));
    fprintf(stderr,"did = %4u ", reg.sw_did);
    fprintf(stderr,"ISA id = %4u ", reg.isa_id);
...
// TODO 把这些注释删掉，把下面的栈处理合并到函数中
// TODO 注意里面大部分是只有 test 才用得到的，需要删除
...
.macro trigger_exception_jump_simple
    j _dm_exception_handler
.align 2
.endm

.macro trigger_exception_call
    SLOW_PUSH ra
    call _dm_exception_handler
.align 2 // uepc needs to be 2-byte aligned
    SLOW_POP ra
.endm
*/
...
// 1f 指紧跟着的“1”的标签
// 用上面的 trigger_exception_call 也行，TODO 把前面定义的四个宏全都删掉
...
// 这里的处理，保留jump ,因为 load 是通过访问异常处理程序的私有数据来触发异常
...
.macro goto_exception_handler id type
    ...
    //la reg_a1_id, 1f
    //la reg_a1_id, _dm_exception_handler
...
dm_\name:
    ...
    lb   t0, PIC(domain_manager_initialized)
    beqz t0, \name
