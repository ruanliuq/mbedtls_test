...
#define TIME_AND_TSC(TITLE, CODE) \
    do { \
        ...
        { \
            ...
            if (elapsed_cycles > 1000000000) { /* Assuming hardclock returns time in microseconds */ \
        ...
        asm volatile("rdcycle %0" : "=r"(end_cycle2));\
        mbedtls_printf("value of jj: %lu\n", jj); \
        fflush(stdout); \
        mbedtls_printf("end_cycle2 time: %llu\n", end_cycle2); \
        fflush(stdout); \
        elapsed_cycles2 = end_cycle2 - start_cycle2;\
        mbedtls_printf("value of elapsed_cycles2: %llu\n", elapsed_cycles2); \
        fflush(stdout); \
        elapsed_cycles2_us = elapsed_cycles2 / 1000 ;\
        \
        if (ret != 0) { \
            PRINT_ERROR; \
        } else { \
            mbedtls_printf("%9lu KiB/s,  %f cycles/byte\n", \
                           ii * BUFSIZE / 1024, \
                           (double)elapsed_cycles2_us  / (jj * BUFSIZE)); \
        } \
    } while (0)
