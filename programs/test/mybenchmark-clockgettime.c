...
#define TIME_AND_TSC(TITLE, CODE) \
    do { \
        unsigned long ii, jj; \
        int ret = 0; \
        long long int elapsed,elapsed_cycle; \
        struct timespec start, current;  \
        struct timespec start_cycle, current_cycle;  \
        struct timespec res;\
        if (clock_getres(CLOCK_REALTIME, &res) == 0) {\
        printf("Clock resolution: %ld sec, %ld nsec\n", res.tv_sec, res.tv_nsec);\
           \
        double frequency = 1.0 / (res.tv_sec + res.tv_nsec / 1e9);\
        printf("Clock frequency: %.2f Hz\n", frequency);\
        } else {\
        ...
        mbedtls_printf("start time: %lld\n", (long long)(start.tv_sec * 1000000000L + start.tv_nsec)); \
        fflush(stdout); \
        mbedtls_printf("start time sec: %ld\n", start.tv_sec); \
        fflush(stdout); \
        mbedtls_printf("start time nsec: %ld\n", start.tv_nsec); \
        fflush(stdout); \
        ...
        { \
            ...
            if(current.tv_nsec < start.tv_nsec){ \
                elapsed = (current.tv_sec - start.tv_sec - 1) * 1000000000L + (current.tv_nsec + 1000000000L - start.tv_nsec); \
                } \
                else{ \
                    elapsed = (current.tv_sec - start.tv_sec) * 1000000000L + (current.tv_nsec - start.tv_nsec);   \
                } \
            if (elapsed > 1000000000) { /* Assuming hardclock returns time in microseconds */ \
                mbedtls_timing_alarmed = 1; \
            } \
        } \
        mbedtls_printf("end time: %lld\n", (long long)(current.tv_sec * 1000000000L + current.tv_nsec)); \
        fflush(stdout); \
        mbedtls_printf("end time sec: %ld\n", current.tv_sec); \
        fflush(stdout); \
        mbedtls_printf("end time nsec: %ld\n", current.tv_nsec); \
        fflush(stdout); \
        ...
        mbedtls_printf("start_cycle time: %lld\n", (long long)(start_cycle.tv_sec * 1000000000L + start_cycle.tv_nsec)); \
        fflush(stdout); \
        mbedtls_printf("start_cycle time sec: %ld\n", start_cycle.tv_sec); \
        fflush(stdout); \
        mbedtls_printf("start_cycle time nsec: %ld\n", start_cycle.tv_nsec); \
        fflush(stdout); \
        for (jj = 0; ret == 0 && jj < 1024; jj++) \
        { \
            ret = CODE; \
        } \
        mbedtls_printf("value of jj: %lu\n", jj); \
        fflush(stdout); \
        clock_gettime(CLOCK_REALTIME, &current_cycle); \
        mbedtls_printf("end_cycle time: %lld\n", (long long)(current_cycle.tv_sec * 1000000000L + current_cycle.tv_nsec)); \
        fflush(stdout); \
        mbedtls_printf("end_cycle time sec: %ld\n", current_cycle.tv_sec); \
        fflush(stdout); \
        mbedtls_printf("end_cycle time nsec: %ld\n", current_cycle.tv_nsec); \
        fflush(stdout); \
        if(current_cycle.tv_nsec < start_cycle.tv_nsec){ \
                elapsed_cycle = (current_cycle.tv_sec - start_cycle.tv_sec - 1) * 1000000000L + (current_cycle.tv_nsec + 1000000000L - start_cycle.tv_nsec); \
                } \
                else{ \
                    elapsed_cycle = (current_cycle.tv_sec - start_cycle.tv_sec) * 1000000000L + (current_cycle.tv_nsec - start_cycle.tv_nsec);   \
                } \
        \
        if (ret != 0) { \
            PRINT_ERROR; \
            ...
            mbedtls_printf("%9lu KiB/s,  %f cycles/byte\n", \
                           ii * BUFSIZE / 1024, \
                           (double)elapsed_cycle / (jj * BUFSIZE) / 1000L); \
        } \
    } while (0)
...
int test_bench(void)
{
    ...
    if (todo.md5) {
        TIME_AND_TSC("MD5", mbedtls_md5(buftest, BUFSIZE, tmp));
    }
#endif

#if defined(MBEDTLS_RIPEMD160_C)
    if (todo.ripemd160) {
        TIME_AND_TSC("RIPEMD160", mbedtls_ripemd160(buftest, BUFSIZE, tmp));
    }
#endif

#if defined(MBEDTLS_SHA1_C)
    if (todo.sha1) {
        TIME_AND_TSC("SHA-1", mbedtls_sha1(buftest, BUFSIZE, tmp));
    }
#endif

#if defined(MBEDTLS_SHA256_C)
    if (todo.sha256) {
    TIME_AND_TSC("SHA-256", mbedtls_sha256(buftest, BUFSIZE, tmp, 0));
    }
#endif

#if defined(MBEDTLS_SHA512_C)
    if (todo.sha512) {
        TIME_AND_TSC("SHA-512", mbedtls_sha512(buftest, BUFSIZE, tmp, 0));
    }
#endif
#if defined(MBEDTLS_SHA3_C)
    if (todo.sha3_224) {
        TIME_AND_TSC("SHA3-224", mbedtls_sha3(MBEDTLS_SHA3_224, buftest, BUFSIZE, tmp, 28));
    }
    if (todo.sha3_256) {
        TIME_AND_TSC("SHA3-256", mbedtls_sha3(MBEDTLS_SHA3_256, buftest, BUFSIZE, tmp, 32));
    }
    if (todo.sha3_384) {
        TIME_AND_TSC("SHA3-384", mbedtls_sha3(MBEDTLS_SHA3_384, buftest, BUFSIZE, tmp, 48));
    }
    if (todo.sha3_512) {
        TIME_AND_TSC("SHA3-512", mbedtls_sha3(MBEDTLS_SHA3_512, buftest, BUFSIZE, tmp, 64));
    }
#endif
    ...
    if (todo.des3) {
        ...
        TIME_AND_TSC("3DES",
                     mbedtls_des3_crypt_cbc(&des3, MBEDTLS_DES_ENCRYPT, BUFSIZE, tmp, buftest, buftest));
    ...
    if (todo.des) {
        ...
        TIME_AND_TSC("DES",
                     mbedtls_des_crypt_cbc(&des, MBEDTLS_DES_ENCRYPT, BUFSIZE, tmp, buftest, buftest));
    ...
    if (todo.des3_cmac) {
        ...
        TIME_AND_TSC("3DES-CMAC",
                     mbedtls_cipher_cmac(cipher_info, tmp, 192, buftest,
                                         BUFSIZE, output));
    }
    ...
    if (todo.aes_cbc) {
        ...
        for (keysize = 128; keysize <= 256; keysize += 64) {
            ...
            TIME_AND_TSC(title,
                         mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_ENCRYPT, BUFSIZE, tmp, buftest, buftest));
        }
    ...
    if (todo.aes_cfb8) {
        ...
        for (keysize = 128; keysize <= 256; keysize += 64) {
            ...
            TIME_AND_TSC(title,
                         mbedtls_aes_crypt_cfb8(&aes, MBEDTLS_AES_ENCRYPT, BUFSIZE, tmp, buftest, buftest));
        }
        mbedtls_aes_free(&aes);
    }
#endif
        ...
        for (keysize = 128; keysize <= 256; keysize += 64) {
            ...
            TIME_AND_TSC(title,
                         mbedtls_gcm_crypt_and_tag(&gcm, MBEDTLS_GCM_ENCRYPT, BUFSIZE, tmp,
                                                   12, NULL, 0, buftest, buftest, 16, tmp));
        ...
        for (keysize = 128; keysize <= 256; keysize += 64) {
            ...
            TIME_AND_TSC(title,
                         mbedtls_ccm_encrypt_and_tag(&ccm, BUFSIZE, tmp,
                                                     12, NULL, 0, buftest, buftest, tmp, 16));
    ...
    if (todo.chachapoly) {
        ...
        TIME_AND_TSC(title,
                     mbedtls_chachapoly_encrypt_and_tag(&chachapoly,
                                                        BUFSIZE, tmp, NULL, 0, buftest, buftest, tmp));
        ...
        for (keysize = 128, cipher_type = MBEDTLS_CIPHER_AES_128_ECB;
            ...
            TIME_AND_TSC(title,
                         mbedtls_cipher_cmac(cipher_info, tmp, keysize,
                                             buftest, BUFSIZE, output));
        }

        memset(buftest, 0, sizeof(buftest));
        memset(tmp, 0, sizeof(tmp));
        TIME_AND_TSC("AES-CMAC-PRF-128",
                     mbedtls_aes_cmac_prf_128(tmp, 16, buftest, BUFSIZE,
                                              output));
    }
#endif /* MBEDTLS_CMAC_C */
#endif /* MBEDTLS_AES_C */
    ...
    if (todo.ctr_drbg) {
        ...
        TIME_AND_TSC("CTR_DRBG (NOPR)",
                     mbedtls_ctr_drbg_random(&ctr_drbg, buftest, BUFSIZE));
    ...
    if (todo.dhm) {
        ...
        for (i = 0; (size_t) i < sizeof(dhm_sizes) / sizeof(dhm_sizes[0]); i++) {
            ...
            mbedtls_dhm_make_public(&dhm, (int) n, buftest, n, myrand, NULL);

            if (mbedtls_dhm_read_public(&dhm, buftest, n) != 0) {
                mbedtls_exit(1);
            }

            mbedtls_snprintf(title, sizeof(title), "DHE-%d", dhm_sizes[i]);
            TIME_PUBLIC(title, "handshake",
                        ret |= mbedtls_dhm_make_public(&dhm, (int) n, buftest, n,
                                                       myrand, NULL);
                        ret |=
                            mbedtls_dhm_calc_secret(&dhm, buftest, sizeof(buftest), &olen, myrand, NULL));
    ...
    if (todo.ecdsa) {
        ...
        memset(buftest, 0x2A, sizeof(buftest));
        ...
        for (curve_info = curve_list;
            ...
            TIME_PUBLIC(title,
                        "sign",
                        ret =
                            mbedtls_ecdsa_write_signature(&ecdsa, MBEDTLS_MD_SHA256, buftest,
                                                          curve_info->bit_size,
        ...
        for (curve_info = curve_list;
            ...
            if (mbedtls_ecdsa_genkey(&ecdsa, curve_info->grp_id, myrand, NULL) != 0 ||
                mbedtls_ecdsa_write_signature(&ecdsa, MBEDTLS_MD_SHA256, buftest, curve_info->bit_size,
            ...
            TIME_PUBLIC(title, "verify",
                        ret = mbedtls_ecdsa_read_signature(&ecdsa, buftest, curve_info->bit_size,
                                                           tmp, sig_len));
